...
export type OverloadedSetCommand<T, U, R> = {
  (key: string, ...args: [...T[], Callback<U> | undefined]): R;
  (key: string, record: Record<string, any>, cb?: Callback<U>): R;
}


export type OverloadedListCommand<T, U, R> = (...args: [...T[], Callback<U> | undefined]) => R
...
export interface LowerCommands<R> {
  /**
...
export type  UpperCommands<R> = {
  [K in Uppercase<keyof LowerCommands<R>>]: LowerCommands<R>[Lowercase<K>]
}

export interface Commands<R> extends LowerCommands<R>, UpperCommands<R> {
}
